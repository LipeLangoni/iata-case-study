...
def parse_args():
    parser = argparse.ArgumentParser(description="Process a CSV file from S3 and convert it to Parquet format.")
    parser.add_argument("--s3_bucket", required=True, help="Name of the S3 bucket")
    parser.add_argument("--input_key", required=True, help="S3 key of the input CSV file")
    parser.add_argument("--output_prefix", required=True, help="S3 prefix for the output Parquet files")
    return parser.parse_args()

if __name__ == "__main__":
    # Initialize Spark session
    spark = SparkSession.builder \
        .appName("CSV to Parquet in SageMaker") \
        .getOrCreate()
    ...
    df = df.select([col(column).alias(column.replace(" ", "_")) for column in df.columns])
