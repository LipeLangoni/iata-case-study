...
def test_script_execution():
    ...
    assert (
        expected_file_key in files_in_s3
    ), f"File {expected_file_key} was not found in S3 bucket {s3_bucket}"
    ...
    assert (
        len(csv_files_in_raw) > 0
    ), f"No CSV files found in the {output_prefix} folder in S3 bucket {s3_bucket}"
