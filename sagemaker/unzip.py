        ...
        for file_name in z.namelist():
            if file_name.endswith(".csv"):
                ...
                s3_client.put_object(
                    Bucket=bucket_name, Key=output_key, Body=file_content
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Unzip an S3 object and save the contents back to S3."
    )
    ...
    parser.add_argument(
        "--output_prefix", required=True, help="S3 prefix for unzipped files"
    )
